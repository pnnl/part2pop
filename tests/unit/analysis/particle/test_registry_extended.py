...
def test_register_particle_variable_and_aliases(monkeypatch):
    ...
    assert "foo" in reg._PARTICLE_REG
    assert reg.resolve_particle_name("bar") == "foo"

    with pytest.raises(KeyError):
        reg.register_particle_variable("foo")(Dummy)
    with pytest.raises(KeyError):
        reg.register_particle_variable("baz")(Dummy)  # alias collision


def test_unknown_particle_variable_suggests(monkeypatch):
    _reset_registry(monkeypatch)
    reg._PARTICLE_REG["abc"] = lambda cfg=None: cfg
    with pytest.raises(reg.UnknownParticleVariableError) as excinfo:
        reg.resolve_particle_name("abd")
