...
from part2pop.viz.factory.state_line import StateLinePlotter, _as_1d
...
def test_as_1d_squeezes_singleton_2d_outputs():
    np.testing.assert_allclose(_as_1d(np.array([[1.0, 2.0, 3.0]])), [1.0, 2.0, 3.0])
    np.testing.assert_allclose(_as_1d(np.array([[1.0], [2.0], [3.0]])), [1.0, 2.0, 3.0])

def test_as_1d_rejects_true_2d_outputs():
    with pytest.raises(ValueError, match="state_line needs 1-D data"):
        _as_1d(np.ones((2, 3)))