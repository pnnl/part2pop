...
@register_particle_variable("ext_crossect")
class ExtCrossSect(ParticleVariable):
    meta = VariableMeta(
        name="ext_crossect",
        description="particle extinction cross section",
        units="m^2",
        axis_names=("particle",),
        default_cfg={
        ...
        aliases=("Cext", "ext_cross_section"),
        scale="log",
        short_label="C_{ext}",
        long_label="extinction cross section",
    )
    ...
    def compute_all(self, population):
        ...
        return np.asarray(optical_pop.Cext[:, 0, 0], dtype=float)


def build(cfg=None):
    cfg = cfg or {}
    return ExtCrossSect(cfg)
