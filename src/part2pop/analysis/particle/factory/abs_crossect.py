...
@register_particle_variable("abs_crossect")
class AbsCrossSect(ParticleVariable):
    meta = VariableMeta(
        name="abs_crossect",
        description="particle absorption cross section",
        units="m^2",
        axis_names=("particle",),
        default_cfg={
        ...
        aliases=("Cabs", "abs_cross_section"),
        scale="log",
        short_label="C_{abs}",
        long_label="absorption cross section",
    )
    ...
    def compute_all(self, population):
        ...
        return np.asarray(optical_pop.Cabs[:, 0, 0], dtype=float)


def build(cfg=None):
    cfg = cfg or {}
    return AbsCrossSect(cfg)
