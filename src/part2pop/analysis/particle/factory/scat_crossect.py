...
@register_particle_variable("scat_crossect")
class ScatCrossSect(ParticleVariable):
    meta = VariableMeta(
        name="scat_crossect",
        description="particle scattering cross section",
        units="m^2",
        axis_names=("particle",),
        default_cfg={
        ...
        aliases=("Csca", "scat_cross_section"),
        scale="log",
        short_label="C_{scat}",
        long_label="scattering cross section",
    )
    ...
    def compute_all(self, population):
        ...
        return np.asarray(optical_pop.Csca[:, 0, 0], dtype=float)


def build(cfg=None):
    cfg = cfg or {}
    return ScatCrossSect(cfg)
