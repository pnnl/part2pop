...
def _read_fims_avg_size_dist(
    *,
    ...
    fims_idx = _time_indices_for_altitude_and_cloudflag(
        aimms=aimms, size_dist=fims, z=z, dz=dz, cloud_flag_value=cloud_flag_value,
        region_filter=region_filter, aimms_time_col=aimms_time_col, aimms_alt_col=aimms_alt_col,
        aimms_lat_col=aimms_lat_col, aimms_lon_col=aimms_lon_col, aimms_cloud_col=aimms_cloud_col,
        size_dist_time_col=fims_time_col, size_dist_cloud_col=fims_cloud_col,
    )
    if fims_idx.size == 0:
        raise RuntimeError("No matching FIMS indices found for requested altitude/region/cloud filters.")
    Nbins = np.asarray(fims["_N_bins"][fims_idx, :], dtype=float)
    ...
    Dp_lo_nm = Dp_lo_nm[keep]; Dp_hi_nm = Dp_hi_nm[keep]
    avg_density = avg_density[keep]; std_density = std_density[keep]
    measure = (fims_density_measure or "ln").strip().lower()
    if measure == "per_bin":
        N_bin = avg_density; N_bin_std = std_density
    else:
        ...
        if measure in ("ln", "dndln", "dndlnd"):
            N_bin = avg_density * dln; N_bin_std = std_density * dln
...
def _read_beasd_avg_size_dist(
    *, beasd_file: str, aimms_file: str, z: float, dz: float,
    cloud_flag_value: Optional[float] = 0, max_dp_nm: float = 1000.0,
    region_filter: Optional[Dict[str, float]] = None, aimms_time_col: str = "Time(UTC)",
    aimms_alt_col: str = "Alt", aimms_lat_col: str = "Lat", aimms_lon_col: str = "Lon",
    aimms_cloud_col: Optional[str] = None, beasd_time_col: Optional[str] = None,
    beasd_cloud_col: str = "Cloud_flag", beasd_density_measure: str = "per_bin",
) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    aimms = _read_aimms(aimms_file)
    ...
    beasd_idx = _time_indices_for_altitude_and_cloudflag(
        aimms=aimms, size_dist=beasd, z=z, dz=dz, cloud_flag_value=cloud_flag_value,
        region_filter=region_filter, aimms_time_col=aimms_time_col, aimms_alt_col=aimms_alt_col,
        aimms_lat_col=aimms_lat_col, aimms_lon_col=aimms_lon_col, aimms_cloud_col=aimms_cloud_col,
        size_dist_time_col=beasd_time_col, size_dist_cloud_col=beasd_cloud_col,
    )
    if beasd_idx.size == 0:
        raise RuntimeError("No matching BEASD indices found for requested altitude/region/cloud filters.")
    Nbins = np.asarray(beasd["_N_bins"][beasd_idx, :], dtype=float)
    avg_density = np.nanmean(Nbins, axis=0); std_density = np.nanstd(Nbins, axis=0)
    nbins = avg_density.size
    ...
    *,
    ...
    if splat_time_col not in splat:
        if "Time(UTC)" in splat:
            ...
            raise KeyError(f"miniSPLAT missing time column '{splat_time_col}'. Available: {sorted(splat.keys())[:50]}")
    size_dist_idx = _time_indices_for_altitude_and_cloudflag(
        aimms=aimms, size_dist=size_dist, z=z, dz=dz, cloud_flag_value=cloud_flag_value,
        region_filter=region_filter, aimms_time_col=aimms_time_col, aimms_alt_col=aimms_alt_col,
        aimms_lat_col=aimms_lat_col, aimms_lon_col=aimms_lon_col, aimms_cloud_col=aimms_cloud_col,
        size_dist_time_col=size_dist_time_col, size_dist_cloud_col=size_dist_cloud_col,
    )
    ...
    for reduced, cols in splat_species.items():
        ...
        for c in cols:
            if c not in splat:
                raise KeyError(f"miniSPLAT missing column '{c}' referenced by splat_species['{reduced}']. Available (sample): {sorted(splat.keys())[:50]}")
...
def _read_ams_mass_fractions(
    *, ams_file: str, aimms_file: str, size_dist_type: str, size_dist_file: str,
    z: float, dz: float, cloud_flag_value: Optional[float] = 0,
    region_filter: Optional[Dict[str, float]] = None, aimms_time_col: str = "Time(UTC)",
    aimms_alt_col: str = "Alt", aimms_lat_col: str = "Lat", aimms_lon_col: str = "Lon",
    aimms_cloud_col: Optional[str] = None, size_dist_time_col: Optional[str] = None,
    size_dist_cloud_col: str = "Cloud_flag", ams_time_col: Optional[str] = None,
    ...
    if ams_time_col is None:
        for cand in ("dat_ams_utc", "Time", "Time(UTC)", "Start_UTC", "UTC", "time"):
            if cand in ams:
                ...
                break
    if ams_time_col is None:
        raise KeyError(f"Could not identify AMS time column. Set config['ams_time_col']. Available (sample): {sorted(ams_cols)[:50]}")
    if ams_flag_col is None:
        for cand in ("flag", "Flag", "qc_flag", "QC_Flag", "qc", "QC"):
            if cand in ams:
                ...
                break
    if ams_flag_col is None:
        raise KeyError(f"Could not identify AMS flag/QC column. Set config['ams_flag_col']. Available (sample): {sorted(ams_cols)[:50]}")
    ...
    if missing:
        raise KeyError(f"AMS missing required mass columns {missing}. Available (sample): {sorted(ams_cols)[:60]}")
    ...
    frac_org = org / total; frac_no3 = no3 / total; frac_so4 = so4 / total; frac_nh4 = nh4 / total
    mass_frac = {"OC": float(np.nanmean(frac_org)), "NO3": float(np.nanmean(frac_no3)), "SO4": float(np.nanmean(frac_so4)), "NH4": float(np.nanmean(frac_nh4))}
    mass_frac_err = {"OC": float(np.nanstd(frac_org)), "NO3": float(np.nanstd(frac_no3)), "SO4": float(np.nanstd(frac_so4)), "NH4": float(np.nanstd(frac_nh4))}
    s = sum(mass_frac.values())
    if s > 0:
        for k in list(mass_frac.keys()):
            mass_frac[k] /= s; mass_frac_err[k] /= s
...
# -----------------------------
# Composition / math / sampling / diagnostics helpers
# -----------------------------
    ...
    *,
    ...
    for line in range(len(mode_fractions)):
        ...
        spec_Ns=size_dependent_composition(measured_Dp, measured_N, 1,
                                               splat_cutoff_nm,
                                               splat_number_fraction=splat_number_fractions['BC'],
                                               mode_fractions=[1.0], size_dist_params=params)
        total_Ns+=spec_Ns

        params=[splat_number_fractions['OIN'], Dpg_dust, sigma_dust]
        spec_Ns=size_dependent_composition(measured_Dp, measured_N, 1,
                                               splat_cutoff_nm,
                                               splat_number_fraction=splat_number_fractions['OIN'],
                                               mode_fractions=[1.0], size_dist_params=params)
        total_Ns+=spec_Ns
